...
SIO_SERVER = 'http://192.168.0.48:3000'  # 이미지 전송 서버 (전역으로 이동)

# --- 관심 영역(ROI) 및 중심점 ---
R_X1, R_Y1, R_X2, R_Y2 = 50, 0, 600, 350
C_X1, C_Y1, C_X2, C_Y2 = 150, 0, 500, 350
...
def send_to_server(car_number, frame_to_send):
    ...
    try:
        ...
        response = requests.post(f"{HTTP_ADDRESS}/entry", params={"car_number": car_number}, timeout=5) # 5초 타임아웃
        ...
        if sio.connected:
            ...
            print(img_b64)
            data = {"car_number": car_number, "image": img_b64}
            sio.emit("entry_photo", data)
            print("... 이미지 전송 완료.")
...
def main_logic():
    ...
    while running:
        ...
        print(history)
        print(last_sent_number)
        print(stable_text)
        now = time.time()
        if detect and (now - last_seen_time > HOLD_SECONDS):
        ...
        if stable_text is None:
            if (now - last_seen_time > HOLD_SECONDS) and (last_sent_number is not None):
                history.clear()
...
if __name__ == '__main__':
    ...
    try:
        sio.connect(SIO_SERVER, transports=['websocket', 'polling'])
